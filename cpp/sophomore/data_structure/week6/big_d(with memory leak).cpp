...
class BigDecimal
{
  ...
  BigDecimal* operator^(const BigDecimal *input) const
  {
    BigDecimal num_1("1","0"), num_2("2","0"), *buf;
    BigDecimal int_input(input->ch_int_arr, "0"), *iter = new BigDecimal("0","0");
    BigDecimal *int_res = new BigDecimal("1","0");
    ...
    if(input->ch_int_arr != "0")
    {
      ...
      }while(!(int_input.ch_int_arr == iter->ch_int_arr));
